...
docker build --no-cache --progress=plain --build-arg VERSION=0.9.3 --build-arg ORACLE_VERSION=${ORACLE_VERSION} --build-arg TARGETPLATFORM="linux/amd64" -t ghcr.io/pfc-developer/kujira-price-feeder:${ORACLE_VERSION} .
# --no-cache
